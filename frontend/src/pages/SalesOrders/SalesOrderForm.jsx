  ...
  return (
    <div ref={formSectionRef} tabIndex={-1} className="bg-white border rounded-xl shadow-sm p-5 mb-6 focus:outline-none focus:ring-2 focus:ring-blue-400">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">
            {editId ? "Edit Sales Order" : "Add Sales Order"}
          </h2>
          <p className="text-sm text-gray-500">
            Create and update customer sales orders with item, quantity, and delivery status.
          </p>
        </div>
      ...
      <div className="mb-4 flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-600">
        <span>{isOpen ? "Sales order form is open" : "Sales order form is hidden"}</span>
      ...
      {isOpen && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <div>
              {renderLabel("Customer", true)}
              <select
                value={formData.customer}
                onChange={(e) => onChange("customer", e.target.value)}
                className={inputClassName("customer")}
              >
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`${inputClassName("date")} pr-12`}
                />
                <button
                  type="button"
